...
export default function Register() {
    ...
    return (
        <div className="min-h-screen bg-stone-900 flex items-center justify-center px-4">
            ...
            >
                <h2 className="text-3xl font-serif text-heritage-gold mb-6 text-center">Join the Journey</h2>
                {error && <div className="bg-red-900/50 text-red-200 p-3 rounded mb-4 text-sm border border-red-800">{error}</div>}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-stone-400 mb-1 text-sm">Username</label>
                        <input
                            type="text"
                            value={username}
                ...
                <p className="mt-4 text-center text-stone-500 text-sm">
                    Already have an account? <Link to="/login" className="text-heritage-gold hover:underline">Login</Link>
                </p>
            </motion.div>
        </div>
    );
